...
export function AuthButton({ user }) {
  const handleSignIn = () => { if (auth) signInWithPopup(auth, googleProvider).catch(console.error) }
  ...
  return (
    <button className="btn auth-signin-btn" onClick={handleSignIn}>
      ...
      <span className="auth-btn-label">Entrar com Google</span>
    </button>
  )
}
